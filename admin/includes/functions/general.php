...
// Parse and secure the hPath parameter values
  function tep_parse_header_path($hPath) {
// make sure the header IDs are integers
    $hPath_array = array_map('tep_string_to_int', explode('_', $hPath));

// make sure no duplicate header IDs exist which could lock the server in a loop
    $tmp_array = array();
    $n = sizeof($hPath_array);
    for ($i=0; $i<$n; $i++) {
      if (!in_array($hPath_array[$i], $tmp_array)) {
        $tmp_array[] = $hPath_array[$i];
      }
    }
...
// Wrapper function for is_writable() for Windows compatibility
  ...
  function tep_remove_header($header_id) {
    $header_image_query = tep_db_query("select headers_class from " . TABLE_HEADERS . " where headers_id = '" . (int)$header_id . "'");
    $header_image = tep_db_fetch_array($header_image_query);

    $duplicate_image_query = tep_db_query("select count(*) as total from " . TABLE_HEADERS . " where headers_class = '" . tep_db_input($header_image['headers_class']) . "'");
    $duplicate_image = tep_db_fetch_array($duplicate_image_query);

    if ($duplicate_image['total'] < 2) {
      if (file_exists(DIR_FS_CATALOG_IMAGES . $header_image['headers_class'])) {
        @unlink(DIR_FS_CATALOG_IMAGES . $header_image['headers_class']);
      }
    }

    tep_db_query("delete from " . TABLE_HEADERS . " where headers_id = '" . (int)$header_id . "'");
    tep_db_query("delete from " . TABLE_HEADERS_DESCRIPTION . " where headers_id = '" . (int)$header_id . "'");
