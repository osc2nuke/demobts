      ...
      case 'status_disable':
        if (isset($_POST['form_type'])) $headers_id = tep_db_prepare_input($_POST['form_type']);         
		 if ($action == 'status_enable') {
            tep_db_query("update " . TABLE_HEADERS . " set headers_status = '1', last_modified = now() where headers_id = '" . (int)$headers_id . "'");		 
		 }
		 if ($action == 'status_disable') {
            tep_db_query("update " . TABLE_HEADERS . " set headers_status = '0', last_modified = now() where headers_id = '" . (int)$headers_id . "'");		 
		 }
              return_content();
        break;
