<?php
    ...
    */
    ...
    if (tep_session_is_registered('customer_id')) {
        ...
        if (!tep_session_is_registered('template_id') || isset($HTTP_GET_VARS['template_id']) ) {
            $template_id = tep_get_default_template((int)$customer_id);
            
            if (!tep_session_is_registered('template_id')) tep_session_register('template_id');
        ...
        $category_query = tep_db_query("select h.headers_id, hd.headers_name from " . TABLE_HEADERS . " h, " . TABLE_HEADERS_DESCRIPTION . " hd  where h.headers_id = hd.headers_id and hd.language_id = '" . (int)$languages_id . "' and h.headers_id = '" . (int)$template_id . "' and h.customers_id = '" . (int)$customer_id . "'");
        $category = tep_db_fetch_array($category_query);
        $category_query2 = tep_db_query("select selectors_id from " . TABLE_BTS_CSS_SELECTORS . "  where  selectors_type = 'template' and selectors_name = '" . $category['headers_name'] . "' and customers_id = '" . (int)$customer_id . "'");
        $category2 = tep_db_fetch_array($category_query2);   
        $css_id = $category2['selectors_id'];
        // echo '-----------'.$css_id;
    }
    ...
    if (isset($HTTP_GET_VARS['action'])) {
        ...
        switch ($HTTP_GET_VARS['action']) {
            ...
            case 'copy_category_and_sub_confirm': if (tep_session_is_registered('customer_id')) {
    ...
    // include BTS Menu Generator class
    require(DIR_WS_CLASSES . 'bts_menu.php'); 
    // entry/item info classes
    require(DIR_WS_CLASSES . 'object_info.php');
    // include category tree class
    require(DIR_WS_CLASSES . 'category_demo_tree.php'); 
    require(DIR_WS_CLASSES . 'status_tree.php');
    require(DIR_WS_CLASSES . 'bts_css_tree.php');     
    require(DIR_WS_CLASSES . 'bts_css_editor.php');   
