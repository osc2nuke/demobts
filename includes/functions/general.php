...
// Return a customer greeting
  function tep_customer_greeting() {
    ...
    $template_check_query = tep_db_query("select count(*) as count from " . TABLE_TEMPLATES_TO_CUSTOMERS . "  where customer_id = '" . (int)$customer_id . "'");
    $template_check = tep_db_fetch_array($template_check_query);

	if($template_check['count'] > 0){
	$greeting_string = sprintf(TEXT_GREETING_PERSONAL, tep_output_string_protected($customer_first_name), tep_href_link(FILENAME_BTS_HEADER_BUILDER));
}else{
	$greeting_string = '<div class="alert alert-danger text-center">PLEASE FIRST GO TO ONE OF THE PRODUCTS AND CLICK ADD TO WORKSTATION</div>';
...
// Cant be used in safe mode.
  ...
  // Sets the status of a header
  function tep_set_header_status($headers_id, $status) {
    if ($status == '1') {
      return tep_db_query("update " . TABLE_HEADERS . " set headers_status = '1', last_modified = now() where headers_id = '" . (int)$headers_id . "'");
    } elseif ($status == '0') {
      return tep_db_query("update " . TABLE_HEADERS . " set headers_status = '0', last_modified = now() where headers_id = '" . (int)$headers_id . "'");
    } else {
      return -1;
    }
  }
    function tep_generate_header_path($id, $from = 'header', $headers_array = '', $index = 0) {
    global $languages_id;

    if (!is_array($headers_array)) $headers_array = array();

    if ($from == 'header') {
      $header_query = tep_db_query("select cd.headers_name, c.parent_id from " . TABLE_HEADERS . " c, " . TABLE_HEADERS_DESCRIPTION . " cd where c.headers_id = '" . (int)$id . "' and c.headers_id = cd.headers_id and cd.language_id = '" . (int)$languages_id . "'");
      $header = tep_db_fetch_array($header_query);
      $headers_array[$index][] = array('id' => $id, 'text' => $header['headers_name']);
      if ( (tep_not_null($header['parent_id'])) && ($header['parent_id'] != '0') ) $headers_array = tep_generate_header_path($header['parent_id'], 'header', $headers_array, $index);
    
}
    return $headers_array;
  }

  function tep_output_generated_header_path($id, $from = 'header') {
    $calculated_header_path_string = '';
    $calculated_header_path = tep_generate_header_path($id, $from);
    for ($i=0, $n=sizeof($calculated_header_path); $i<$n; $i++) {
      for ($j=0, $k=sizeof($calculated_header_path[$i]); $j<$k; $j++) {
        $calculated_header_path_string .= $calculated_header_path[$i][$j]['text'] . '&nbsp;&gt;&nbsp;';
      }
      $calculated_header_path_string = substr($calculated_header_path_string, 0, -16) . '<br />';
    }
    $calculated_header_path_string = substr($calculated_header_path_string, 0, -6);

    if (strlen($calculated_header_path_string) < 1) $calculated_header_path_string = TEXT_TOP;

    return $calculated_header_path_string;
  }

  function tep_get_generated_header_path_ids($id, $from = 'header') {
    $calculated_header_path_string = '';
    $calculated_header_path = tep_generate_header_path($id, $from);
    for ($i=0, $n=sizeof($calculated_header_path); $i<$n; $i++) {
      for ($j=0, $k=sizeof($calculated_header_path[$i]); $j<$k; $j++) {
        $calculated_header_path_string .= $calculated_header_path[$i][$j]['id'] . '_';
      }
      $calculated_header_path_string = substr($calculated_header_path_string, 0, -1) . '<br />';
    }
    $calculated_header_path_string = substr($calculated_header_path_string, 0, -6);
  ...
  function tep_remove_header($header_id, $customer_id) {
    $header_verify_remove = tep_db_query("select headers_id from " . TABLE_HEADERS . " where headers_id = '" . (int)$header_id . "' and customers_id = '" . (int)$customer_id . "'");
    $header_remove = tep_db_fetch_array($header_verify_remove);
    $verified_header_id = $header_remove['headers_id'];


    tep_db_query("delete from " . TABLE_HEADERS . " where headers_id = '" . (int)$verified_header_id . "' and customers_id = '" . (int)$customer_id . "'");
    tep_db_query("delete from " . TABLE_HEADERS_DESCRIPTION . " where headers_id = '" . (int)$verified_header_id . "'");


  }
  function tep_get_header_name($header_id, $language_id) {
    $header_query = tep_db_query("select headers_name from " . TABLE_HEADERS_DESCRIPTION . " where headers_id = '" . (int)$header_id . "' and language_id = '" . (int)$language_id . "'");
    $header = tep_db_fetch_array($header_query);

    return $header['headers_name'];
  }
  function tep_get_header_type($header_id) {
    $header_query = tep_db_query("select headers_type from " . TABLE_HEADERS . " where headers_id = '" . (int)$header_id . "'");
    $header = tep_db_fetch_array($header_query);

    return $header['headers_type'];
  }  
  function tep_header_get_path($current_header_id = '') {
    global $hPath_array;

    if ($current_header_id == '') {
      $hPath_new = implode('_', $hPath_array);
    } else {
      if (sizeof($hPath_array) == 0) {
        $hPath_new = $current_header_id;
      } else {
        $hPath_new = '';
        $last_header_query = tep_db_query("select parent_id from " . TABLE_HEADERS . " where headers_id = '" . (int)$hPath_array[(sizeof($hPath_array)-1)] . "'");
        $last_header = tep_db_fetch_array($last_header_query);

        $current_header_query = tep_db_query("select parent_id from " . TABLE_HEADERS . " where headers_id = '" . (int)$current_header_id . "'");
        $current_header = tep_db_fetch_array($current_header_query);

        if ($last_header['parent_id'] == $current_header['parent_id']) {
          for ($i = 0, $n = sizeof($hPath_array) - 1; $i < $n; $i++) {
            $hPath_new .= '_' . $hPath_array[$i];
          }
        } else {
        ...
        $hPath_new .= '_' . $current_header_id;

        if (substr($hPath_new, 0, 1) == '_') {
          $hPath_new = substr($hPath_new, 1);
        }
      }
    }

    return 'hPath=' . $hPath_new;
  } 
////
...
// Count how many subheaders exist in a headers
// TABLES: headers
  function tep_childs_in_header_count($headers_id) {
    $headers_count = 0;

    $headers_query = tep_db_query("select headers_id from " . TABLE_HEADERS . " where parent_id = '" . (int)$headers_id . "'");
    while ($headers_id = tep_db_fetch_array($headers_query)) {
      $headers_count++;
      $headers_count += tep_childs_in_header_count($headers_id['headers_id']);
    }
  ...
  function tep_get_header_tree($parent_id = '0', $spacing = '', $exclude = '', $header_tree_array = '', $include_itself = false) {
    ...
    if (!is_array($header_tree_array)) $header_tree_array = array();
    if ( (sizeof($header_tree_array) < 1) && ($exclude != '0') ) $header_tree_array[] = array('id' => '0', 'text' => TEXT_TOP);
    ...
    while ($headers = tep_db_fetch_array($headers_query)) {
      if ($exclude != $headers['headers_id']) $header_tree_array[] = array('id' => $headers['headers_id'], 'text' => $spacing . $headers['headers_name']);
  ...
  ////////header builder end
  ////css editor start
  function tep_get_css_tree($parent_id = '0', $spacing = '', $exclude = '', $css_tree_array = '', $include_itself = false) {
  
    if (!is_array($css_tree_array)) $css_tree_array = array();

    if ($include_itself) {
      $css_query = tep_db_query("select selectors_name from " . TABLE_BTS_CSS_SELECTORS . "  where selectors_id = '" . (int)$parent_id . "'");
      $css = tep_db_fetch_array($css_query);
      $css_tree_array[] = array('id' => $parent_id, 'text' => $css['selectors_name']);
    }

    $selectors_query = tep_db_query("select selectors_id, selectors_name, parent_id from " . TABLE_BTS_CSS_SELECTORS . " where selectors_id = selectors_id and parent_id = '" . (int)$parent_id . "' order by sort_order");
    while ($selectors = tep_db_fetch_array($selectors_query)) {
      if ($exclude != $selectors['selectors_id']) $css_tree_array[] = array('id' => $selectors['selectors_id'], 'text' => $spacing . $selectors['selectors_name']);
      $css_tree_array = tep_get_css_tree($selectors['selectors_id'], $spacing . '&nbsp;&nbsp;&nbsp;', $exclude, $css_tree_array);
    }

    return $css_tree_array;
  }  
...
//iBridge 2010-11-02 copy ALL categories & sub categories
/////////////////////////////////////////////
  function tep_get_default_template($customer_id) {
    global $customer_id, $languages_id;

    $categories_query = tep_db_query("select c.headers_id from " . TABLE_HEADERS . " c, " . TABLE_HEADERS_DESCRIPTION . " cd where cd.language_id = '" . (int)$languages_id . "' and c.headers_type = 'template' and c.headers_id = cd.headers_id and c.customers_id = '" . (int)$customer_id . "' and cd.headers_name = 'default'");
    $category = tep_db_fetch_array($categories_query);
    return $category['headers_id'];
 }
  ...
  function tep_get_templates_tree($parent_id = '0', $spacing = '', $exclude = '', $category_tree_array = '', $include_itself = false) {
    ...
    if ( (sizeof($category_tree_array) < 1) && ($exclude != '0') ) $category_tree_array[] = array('id' => '', 'text' => MODULE_BOXES_HEADING_TEMPLATES_TITLE);
    ...
    while ($categories = tep_db_fetch_array($categories_query)) {
      if ($exclude != $categories['headers_id']) $category_tree_array[] = array('id' => $categories['headers_id'], 'text' => $spacing . $categories['headers_name']);
