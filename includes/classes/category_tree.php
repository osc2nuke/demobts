 ...
 */
  ...
  class category_tree {
    ...
    public function __construct() {
      ...
      if ( isset($_category_tree_data) ) {
        ...
        while ( $categories = tep_db_fetch_array($categories_query) ) {
          $this->_data[$categories['parent_id']][$categories['categories_id']] = array('categories_name' => $categories['categories_name'],
    ...
    protected function _buildBranch($parent_id, $level = 0) {
      $result = ((($level === 0) && ($this->parent_group_apply_to_root === true)) || ($level === 1)) ? $this->parent_group_start_string : null;
      ...
      if ( isset($this->_data[$parent_id]) ) {
        foreach ( $this->_data[$parent_id] as $category_id => $category ) {
          ...
          */
	//	  if ( ( $level === 0 ) && isset($this->_data[$category_id]) && (($this->max_level == '0') || ($this->max_level > $level+1 ))) {
           // $link_title = $this->cpath_start_string . $category['categories_name'] . $this->cpath_end_string;
         // } else {
            $link_title = $category['categories_name'];
         // }
		  if ( ( $level > 0 ) && isset($this->_data[$category_id]) && (($this->max_level == '0') || ($this->max_level > $level+1 ))) {
          	$result .= '<a href="' . tep_href_link(FILENAME_DEFAULT, 'cPath=' . $category_link) . '">' . $link_title . '</a><ul class="dropdown-menu">';
             }else{
		    $result .= '<a href="' . tep_href_link(FILENAME_DEFAULT, 'cPath=' . $category_link) . '">' . $link_title . '</a>';
          }
      ...
      $result .= ((($level === 0) && ($this->parent_group_apply_to_root === true)) || ($level === 1)) ? $this->parent_group_end_string : null;
      ...
      if (isset($this->_data[$parent_id])) {
        foreach ($this->_data[$parent_id] as $category_id => $category) {
          ...
          $result[] = array('id' => $category_link,
                            'title' => str_repeat($this->spacer_string, $this->spacer_multiplier * $level) . $category['categories_name']);
