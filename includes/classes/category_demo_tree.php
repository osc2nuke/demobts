 ...
 */
 ...
 $data_string = 'data-headers-name="'.strip_tags($category['name']).'" data-class="'.$category['headers_class'].'" data-css-id="'.$category['headers_css_id'].'" data-url="'.$category['headers_url'].'" data-fa-icon="'.$category['headers_fa_icon'].'" data-sort-order="'.$category['sort_order'].'" data-element-id="'.$category_id.'" data-type="'.$category['type'].'" id="'.$category_id.'"'; 
		...
		// if($category['type'] == 'template'){
		 // $result .= '		<!-- template Starts -->';
		 // $result .= '			<div class="col-md-12 breadcrumb alert alert-template dragMe" '.$data_string.' draggable="false">';		  
		 // $result .= '<span class="label label-template">' . $category['type'] . ' : ' . $category['name'] . '</span>';
		  //}
		 ...
		 if($category['type'] == 'language'){
		  $result .= '		<!-- language Starts -->';
		  $result .= '			<div class="col-xs-1 dragMe" '.$data_string.' draggable="true"><i class="action pover fa fa-globe" data-action="edit_header"></i></div>';		  
		  $result .= '		<!-- language Ends -->';
		  }			  
//button sample start
...
//button sample end		  
         ...
         if(($category['type'] == 'div') || ($category['type'] == 'container') || ($category['type'] == 'templateabc') || ($category['type'] == 'row') || ($category['type'] == 'column') || ($category['type'] == 'nav') || ($category['type'] == 'dropdown')){
		    $result .= '			</div>';		  
		    $result .= '		<!-- '.$category['type'].' Ends -->';
		  }
